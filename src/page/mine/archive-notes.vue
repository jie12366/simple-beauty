<template>
    <mine :uid="uid">
        <div>
            <div class="title-main"><span class="title" @click="archiveWall">归档</span><span class="archive"> - {{archive}}</span></div>
        </div>
...
export default {
    ...
    methods: {
        getArticles (index, size) {
            this.$api.articles.getArticlesByArchive(this.uid, this.archive, index, size)
            .then(res => {
                if (res.code === 1) {
